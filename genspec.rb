...
class LogStashConfigDocGenerator
  ...
  def generate(file, settings)
    ...
    puts "Generating SPEC code for #{file}"
    ...
    template_file = File.join(File.dirname(__FILE__), "plugin-spec.erb")
    ...
    if settings[:output]
      ...
      path = File.join(dir, "logstash_#{section}_#{name}_spec.rb")
